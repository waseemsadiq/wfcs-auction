<?php
// Atom: select
// Props: $label, $name, $options (array of ['value'=>..., 'label'=>...]), $selected, $required, $help, $error, $id, $class, $placeholder
$selected    = $selected ?? '';
$id          = $id ?? ($name ?? '');
$class       = $class ?? '';
$options     = $options ?? [];
...
<div>
  ...
  <select
    name="<?= e($name ?? '') ?>"
    id="<?= e($id) ?>"
    class="field w-full px-4 py-3 text-sm bg-white dark:bg-slate-700/50 border <?= !empty($error) ? 'border-red-400 dark:border-red-500' : 'border-slate-200 dark:border-slate-600' ?> rounded-xl text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-colors <?= e($class) ?>"
    <?= !empty($required) ? 'required' : '' ?>
  >
    <?php if ($placeholder): ?>
    <option value="" <?= $selected === '' ? 'selected' : '' ?>><?= e($placeholder) ?></option>
    <?php endif; ?>
    <?php foreach ($options as $opt): ?>
    <option value="<?= e($opt['value']) ?>" <?= (string)$selected === (string)$opt['value'] ? 'selected' : '' ?>><?= e($opt['label']) ?></option>
    <?php endforeach; ?>
  </select>
  <?php if (!empty($error)): ?>
